package com.ankamagames.tiphon.sequence
{
   ...
   public class PlayAnimationStep extends AbstractSequencable
   {
      ...
      override public function start() : void
      {
         ...
         var _loc2_:Array = this._target.getAvaibleDirection(this._animationName,true);
         if(!_loc2_[this._target.getDirection()])
         {
            for(_loc1_ in _loc2_)
            {
               if(_loc2_[_loc1_])
               {
                  this._target.setDirection(uint(_loc1_));
                  break;
               }
            }
         }
      ...
      private function onRemoveFromStage(param1:Event) : void
      {
         ...
         var _loc3_:Boolean = this._target.isPlayingAnimation();
         var _loc4_:Array = this._target.getSubEntitiesList();
         if(!_loc3_ && _loc4_.length > 0)
         {
            for each(_loc2_ in _loc4_)
            {
               if(_loc2_.isPlayingAnimation())
               {
                  _loc3_ = true;
                  break;
               }
