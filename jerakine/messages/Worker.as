package com.ankamagames.jerakine.messages
{
   ...
   public class Worker extends EventDispatcher implements MessageHandler
   {
      ...
      public function clear() : void
      {
         ...
         var _loc2_:Vector.<Frame> = new Vector.<Frame>();
      ...
      private function processMessages() : void
      {
         ...
         while(_loc2_ < MAX_MESSAGES_PER_FRAME && this._messagesQueue.length > 0 && getTimer() - _loc3_ < MAX_TIME_FRAME)
         {
            _loc1_ = this._messagesQueue.shift();
            if(!(_loc1_ is CancelableMessage && CancelableMessage(_loc1_).cancel))
            {
               if(this._paused && _loc1_ is QueueableMessage && !this.msgIsUnstoppable(_loc1_))
               {
                  this._pausedQueue.push(_loc1_);
                  _log.warn("Queued message: " + _loc1_);
               }
               else
               {
                  this.processMessage(_loc1_);
                  if(_loc1_ is Poolable)
                  {
                     GenericPool.free(_loc1_ as Poolable);
                  }
                  this.processFramesInAndOut();
                  _loc2_++;
               }
