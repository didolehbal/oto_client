package com.ankamagames.jerakine.replay
{
   ...
   public class LogFrame implements Frame
   {
      ...
      private function writeObject(param1:ByteArray, param2:*) : uint
      {
         ...
         var _loc4_:LogClassField = null;
         if(param2 == null)
         {
            param1.writeInt(NULL_IDENTIFIER);
            return param1.length;
         }
         var _loc5_:Array = this.getClassField(param2);
         ...
         {
            _loc3_ = this._reverseStringRef[_loc4_.fieldNameId];
            switch(_loc4_.type)
            {
               case INT:
                  if(!_loc4_.transient)
                  {
                     param1.writeInt(param2[_loc3_]);
                  }
                  else
                  {
                     param1.writeInt(0);
               ...
               case UINT:
                  if(!_loc4_.transient)
                  {
                     param1.writeUnsignedInt(param2[_loc3_]);
                  }
                  else
                  {
               ...
               case NUMBER:
                  if(!_loc4_.transient)
                  {
                     param1.writeDouble(param2[_loc3_]);
                  }
                  else
                  {
                     param1.writeDouble(param2[_loc3_]);
                  ...
                  continue;
               case BOOLEAN:
                  param1.writeBoolean(param2[_loc3_]);
                  continue;
               case STRING:
                  param1.writeBoolean(param2[_loc3_] == null);
                  if(param2[_loc3_] != null)
                  {
                     if(!_loc4_.transient)
                     {
                        param1.writeUnsignedInt(this.getStringIndex(param2[_loc3_]));
                     }
                     else
                     {
                        param1.writeUnsignedInt(this.getStringIndex(NO_LOG_STRING.substr(0,param2[_loc3_].length)));
                     }
                  }
                  continue;
               default:
                  this.writeObject(param1,param2[_loc3_]);
                  continue;
            }
         }
         return param1.length;
      ...
      private function getClassField(param1:*) : Array
      {
         ...
         if(param1 is Array || param1 is Dictionary || param1 is Vector.<*> || param1 is Vector.<uint> || param1 is Vector.<Boolean> || param1 is Vector.<int> || param1 is Vector.<Number>)
         {
            ...
            for(_loc4_ in param1)
            {
               ...
               {
                  _loc2_[_loc2_.length] = new LogClassField(this.getStringIndex(_loc4_),this.getClassIndex(getQualifiedClassName(param1[_loc4_])),false);
               }
               else
               {
                  _loc2_[_loc2_.length] = new LogClassField(NULL_IDENTIFIER,this.getClassIndex(getQualifiedClassName(param1[_loc4_])),false);
               }
            }
            this.writeClassDefinition(++this._classCount,_loc5_,_loc3_,_loc2_);
            this._arrayDef[param1] = this._classCount;
            return _loc2_;
         }
         if(!this._classRef[_loc5_])
         {
            this.getClassIndex(_loc5_);
         }
         return this._classRef[_loc5_];
      }
