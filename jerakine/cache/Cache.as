package com.ankamagames.jerakine.cache
{
   ...
   public class Cache
   {
      ...
      public function containsCachable(param1:Class, param2:String) : Boolean
      {
         ...
         if(param2 != "")
         {
            _loc3_ = this.getIndexFromString(this.getStringFromClassAndName(param1,param2));
            if(_loc4_ = this._dicIndexObject[_loc3_] != null || this._dicIndexObject[_loc3_] != undefined)
            {
               this._dicCache[this._dicIndexObject[_loc3_]] = getTimer();
            }
         }
         else
         {
      ...
      private function getStringFromClassAndName(param1:Class, param2:String) : String
      {
         ...
         var _loc4_:Array;
         return (_loc4_ = getQualifiedClassName(param1).split("::"))[1] + "" + param2;
      }
      ...
      private function cleanCache() : Boolean
      {
         ...
         for(_loc1_ in this._dicIndexObject)
         {
            if(!ICachable(this._dicIndexObject[_loc1_]).inUse)
            {
               if(this._dicCache[this._dicIndexObject[_loc1_]] < _loc3_)
               {
                  _loc3_ = this._dicCache[this._dicIndexObject[_loc1_]];
                  _loc2_ = this._dicIndexObject[_loc1_];
               }
            }
         }
         if(_loc2_ != null && _loc2_["name"] != null)
         {
            _log.error("Objet " + _loc2_["name"] + " supprimé du cache.");
            delete this._dicCache[_loc2_];
            delete this._dicIndexObject[this.getIndex(_loc2_)];
