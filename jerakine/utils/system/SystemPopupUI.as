package com.ankamagames.jerakine.utils.system
{
   ...
   public class SystemPopupUI extends Sprite
   {
      ...
      private function buildUI(param1:Boolean = false) : void
      {
         ...
         if(this._callBacks)
         {
            for(_loc2_ in this._callBacks)
            {
               Sprite(_loc2_).removeEventListener(MouseEvent.MOUSE_OVER,this.onBtnMouseOver);
               Sprite(_loc2_).removeEventListener(MouseEvent.MOUSE_OUT,this.onBtnMouseOut);
               Sprite(_loc2_).removeEventListener(MouseEvent.CLICK,this.onBtnClick);
            }
         }
         ...
         var _loc6_:TextFormat = new TextFormat("Verdana",16,this._style_font_color,true);
         this._titleTf.defaultTextFormat = _loc6_;
         ...
         _loc6_ = new TextFormat("Verdana",14,this._style_font_color,null,null,null,null,null,!!this._centerContent?TextFormatAlign.CENTER:TextFormatAlign.LEFT);
         this._contentTf.defaultTextFormat = _loc6_;
         ...
         if(this._buttons && this._buttons.length)
         {
            ...
            {
               _loc5_ = this.createButton(_loc4_.label);
               this._callBacks[_loc5_] = _loc4_.callback;
               _loc5_.x = !!_loc3_.width?Number(_loc3_.width + 10):Number(0);
               _loc3_.addChild(_loc5_);
            }
            _loc3_.y = this._contentTf.y + this._contentTf.height + 10;
            _loc3_.x = (this._mainContainer.width - _loc3_.width) / 2;
            this._mainContainer.addChild(_loc3_);
         }
