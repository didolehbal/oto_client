package com.ankamagames.jerakine.utils.files
{
   ...
   public class ZipLoader extends EventDispatcher implements ICachable
   {
      ...
      private function onLoadComplete(param1:Event) : void
      {
         ...
         var _loc4_:uint = 0;
         var _loc3_:ByteArray = ByteArray(URLLoader(param1.target).data);
         this._zipFile = new ZipFile(_loc3_);
         while(_loc4_ < this._zipFile.entries.length)
         {
            _loc2_ = this._zipFile.entries[_loc4_];
            this._files[_loc2_.name] = _loc2_;
            this._filesNames.push(_loc2_.name);
            _loc4_++;
         }
