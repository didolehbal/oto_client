   ...
   public class Uri
   {
      ...
      public function isSecure() : Boolean
      {
         ...
         try
         {
            ...
            _loc3_ = _loc1_;
            for(; unescape(_loc2_.nativePath) != _loc1_; _loc3_ = _loc3_ + (" -> " + unescape(_loc2_.nativePath)))
            {
      ...
      {
         var _loc1_:String = null;
         var _loc2_:String = unescape(this._path);
         if(SystemManager.getSingleton().os == OperatingSystem.WINDOWS && (_loc2_.indexOf("\\\\") == 0 || _loc2_.charAt(1) == ":"))
         {
            return new File(_loc2_);
         }
         if(SystemManager.getSingleton().os != OperatingSystem.WINDOWS && _loc2_.charAt(0) == "/")
         {
            return new File("/" + _loc2_);
         }
         if(this._protocol == "mod")
         {
            _loc1_ = LangManager.getInstance().getEntry("config.mod.path");
            if(_loc1_.substr(0,2) != "\\\\" && _loc1_.substr(1,2) != ":/")
            {
               return new File(File.applicationDirectory.nativePath + File.separator + _loc1_ + File.separator + _loc2_);
            }
            return new File(_loc1_ + File.separator + _loc2_);
         }
         return new File(File.applicationDirectory.nativePath + File.separator + _loc2_);
      }
