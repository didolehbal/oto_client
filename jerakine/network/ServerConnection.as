package com.ankamagames.jerakine.network
{
   ...
   public class ServerConnection implements IEventDispatcher, IServerConnection
   {
      ...
      public static var DEBUG_VERBOSE:Boolean = true;
      ...
      public function get latencyAvg() : uint
      {
         ...
         for each(_loc1_ in this._latencyBuffer)
         {
            _loc2_ = _loc2_ + _loc1_;
         }
         return _loc2_ / this._latencyBuffer.length;
      }
      ...
      private function inspect(param1:*, param2:String = "", param3:Boolean = false) : String
      {
         ...
         var _loc6_:* = "";
         var _loc7_:Array = DescribeTypeCache.getVariables(param1,true,false);
         if(!param3)
         {
            _loc6_ = _loc6_ + this.getType(param1);
         }
         for each(_loc4_ in _loc7_)
         {
            ...
            switch(true)
            {
               case _loc5_ is Vector.<int>:
               case _loc5_ is Vector.<uint>:
               case _loc5_ is Vector.<Boolean>:
               case _loc5_ is Vector.<String>:
               case _loc5_ is Vector.<Number>:
                  _loc6_ = _loc6_ + (" (" + this.getType(_loc5_) + ", len:" + _loc5_.length + ") = " + _loc5_);
                  continue;
               case getQualifiedClassName(_loc5_).indexOf("Vector") != -1:
               case _loc5_ is Array:
                  _loc6_ = _loc6_ + (" (" + this.getType(_loc5_) + ", len:" + _loc5_.length + ") = " + this.inspect(_loc5_,param2 + "\t",true));
                  continue;
               case _loc5_ is String:
                  _loc6_ = _loc6_ + (" = \"" + _loc5_ + "\"");
                  continue;
               case _loc5_ is uint:
               case _loc5_ is int:
               case _loc5_ is Boolean:
               case _loc5_ is Number:
                  _loc6_ = _loc6_ + (" = " + _loc5_);
                  continue;
               default:
                  _loc6_ = _loc6_ + (" " + this.inspect(_loc5_,param2 + "\t"));
                  continue;
            }
         }
         return _loc6_;
      ...
      private function receive(param1:IDataInput) : void
      {
         ...
         if(this._socket.remoteAddress != this._remoteSrvHost)
         {
            this.close();
            return;
         }
