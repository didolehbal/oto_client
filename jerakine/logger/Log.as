package com.ankamagames.jerakine.logger
{
   ...
   public final class Log
   {
      ...
      private static function parseConfiguration(param1:XML) : void
      {
         ...
         var _loc3_:int = 0;
         var _loc4_:* = config..filter;
         for(; §§hasnext(_loc4_,_loc3_); ltf = new LogTargetFilter(filter.@value,allow),filters.push(ltf))
         {
      ...
      private static function flushBuffer() : void
      {
         ...
         var _loc3_:Array = _tempTarget.getBuffer();
         removeTarget(_tempTarget);
         for each(_loc1_ in _targets)
         {
            if(_loc1_ is TemporaryBufferTarget)
            {
               TemporaryBufferTarget(_loc1_).clearBuffer();
               break;
            }
         }
