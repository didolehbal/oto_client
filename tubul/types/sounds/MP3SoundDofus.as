package com.ankamagames.tubul.types.sounds
{
   ...
   public class MP3SoundDofus implements ISound
   {
      ...
      public function play(param1:Boolean = false, param2:int = 1, param3:VolumeFadeEffect = null, param4:VolumeFadeEffect = null) : void
      {
         ...
         {
            _loc5_ = new Callback(this.play,param1,param2,param3,param4);
            this._onLoadingComplete.push(_loc5_);
            return;
         }
         this._playing = true;
         ...
         this.applyParam();
         var _loc6_:* = "Play / file : " + this._uri.fileName + " / id : " + this._id + " / vol. réel : " + Math.round(this.effectiveVolume * 1000) / 1000 + " / vol. : " + Math.round(this._volume * 1000) / 1000;
         if(this is LocalizedSound)
         {
            _loc6_ = _loc6_ + " / vol. max : " + (this as LocalizedSound).volumeMax;
         }
         _loc6_ = _loc6_ + " / vol. fade : " + Math.round(this._fadeVolume * 1000) / 1000 + " / bus : " + this.bus.name + " / vol. bus : " + Math.round(this.bus.effectiveVolume * 1000) / 1000;
         if(param3 != null)
         {
      ...
      private function processQueueFunction() : void
      {
         ...
         var _loc2_:int = this._onLoadingComplete.length;
         if(_loc2_ > 0)
         {
            this._onLoadingComplete.reverse();
            for each(_loc1_ in this._onLoadingComplete)
            {
               _loc1_.exec();
               _loc1_ = null;
            }
