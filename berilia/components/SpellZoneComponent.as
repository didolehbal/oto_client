package com.ankamagames.berilia.components
{
   ...
   public class SpellZoneComponent extends GraphicContainer implements FinalizableUIComponent
   {
      ...
      public function setSpellLevel(param1:ICellZoneProvider) : void
      {
         ...
         var _loc3_:Boolean = this._spellLevel.minimalRange == 0 && this._spellLevel.maximalRange == 0 || this._spellLevel.maximalRange == 63;
         var _loc5_:Boolean = true;
         for each(_loc2_ in this._spellLevel.spellZoneEffects)
         {
            if(!(_loc2_.zoneSize == 0 && _loc2_.zoneShape == 80))
            {
               _loc5_ = false;
            }
         }
      ...
      private function getCenterCellId(param1:uint) : uint
      {
         ...
         return uint(MapPoint.fromCoords(_loc2_,_loc3_).cellId);
      }
   }
}
