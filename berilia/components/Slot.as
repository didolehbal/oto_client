package com.ankamagames.berilia.components
{
   ...
   public class Slot extends ButtonContainer implements ISlotDataHolder, FinalizableUIComponent, IDragAndDropHandler
   {
      ...
      private function updateTimer(param1:int) : void
      {
         ...
         {
            this._data.endTime = _loc3_ + this._timerMaxDuration;
         ...
         else
         {
            ...
            this._timerStartTime = this._data.startTime;
            _loc2_ = Math.round((_loc3_ - this._timerStartTime) / this._timerMaxDuration * 100);
            _loc2_ = 100 - _loc2_;
            this._tx_timerForeground.gotoAndStop = _loc2_;
         }
      ...
      override public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case param1 is MouseUpMessage:
               ...
               if(_loc2_ && _loc2_.data is SlotDragAndDropData)
               {
                  if((_loc10_ = _loc2_.data).slotData != this._data && this.dropValidator(this,SlotDragAndDropData(_loc2_.data).slotData,_loc10_.currentHolder))
                  {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         var _loc2_:int = 0;
         var _loc3_:int = getTimer();
         if(_loc3_ > this._timerStartTime + this._timerMaxDuration)
         {
            EnterFrameDispatcher.removeEventListener(this.onEnterFrame);
      ...
      private function onDragAndDropStart(param1:Event) : void
      {
         ...
         _loc6_ = (_loc6_ = (_loc6_ = (_loc6_ = (_loc6_ = new Array()).concat([1 / 2,0,0,0,0])).concat([0,1 / 2,0,0,0])).concat([0,0,1 / 2,0,0])).concat([0,0,0,1,0]);
