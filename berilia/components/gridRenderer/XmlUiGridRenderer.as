   ...
   public class XmlUiGridRenderer implements IGridRenderer
   {
      ...
      public function XmlUiGridRenderer(param1:String)
      {
         ...
         if(_loc3_[1])
         {
            ...
            this._bgColor1.color = parseInt(_loc3_[1],16);
         }
         if(_loc3_[2])
         {
            this._bgColor2 = new ColorTransform();
            this._bgColor2.color = parseInt(_loc3_[2],16);
         }
         if(_loc4_.indexOf("::") != -1)
         {
            _loc2_ = _loc4_.split("::");
            if(!UiModuleManager.getInstance().getModules()[_loc2_[0]])
            {
               throw new BeriliaError("Module [" + _loc2_[0] + "] does not exit (grid parameter name [" + _loc4_ + "])");
            }
            this._sUiModule = _loc2_[0];
            if(!UiModule(UiModuleManager.getInstance().getModules()[_loc2_[0]]).uis[_loc2_[1]])
            {
               throw new BeriliaError("Ui [" + _loc2_[1] + "] does not exit in module [" + _loc2_[0] + "] (grid parameter name [" + _loc4_ + "])");
            }
            this._sUiName = _loc2_[1];
         }
         else
         {
            this._sUiName = _loc4_;
         }
      }
