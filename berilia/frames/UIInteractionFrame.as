package com.ankamagames.berilia.frames
{
   ...
   public class UIInteractionFrame implements Frame
   {
         ...
         switch(true)
         {
            ...
            case param1 is HumanInputMessage:
               ...
               for each(_loc14_ in this.hierarchy)
               {
                  ...
                  if(!_loc6_ || _loc7_)
                  {
                     if(_loc15_ = UIComponent(_loc14_).process(_loc5_))
                     {
      ...
      private function processRegisteredUiEvent(param1:Message, param2:Grid) : void
      {
         ...
         var _loc5_:InstanceEvent = InstanceEvent(UIEventManager.getInstance().instances[this.currentDo]);
         var _loc6_:String = EventEnums.convertMsgToFct(getQualifiedClassName(param1));
         ModuleLogger.log(param1,_loc5_.instance);
         if(param2)
         {
            if(param1 is MouseWheelMessage)
            {
               _loc3_ = [SecureCenter.secure(_loc5_.instance),MouseWheelMessage(param1).mouseEvent.delta];
            }
            else
            {
               _loc3_ = [SecureCenter.secure(_loc5_.instance)];
            }
            _loc4_ = param2.renderer.eventModificator(param1,_loc6_,_loc3_,_loc5_.instance as UIComponent);
            ErrorManager.tryFunction(CallWithParameters.call,[_loc5_.callbackObject[_loc6_],_loc3_],"Erreur lors du traitement de " + param1);
         }
         else if(param1 is MouseWheelMessage)
         {
            ErrorManager.tryFunction(_loc5_.callbackObject[_loc6_],[SecureCenter.secure(_loc5_.instance),MouseWheelMessage(param1).mouseEvent.delta]);
         }
         else
         {
            ErrorManager.tryFunction(_loc5_.callbackObject[_loc6_],[SecureCenter.secure(_loc5_.instance)]);
         }
      }
