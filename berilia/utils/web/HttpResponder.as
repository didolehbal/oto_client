package com.ankamagames.berilia.utils.web
{
   ...
   public class HttpResponder
   {
      ...
      public function HttpResponder(param1:Socket, param2:String, param3:String, param4:String)
      {
         ...
         var _loc8_:File;
         if((_loc8_ = new File(param4)).exists)
         {
            _loc6_ = File.applicationDirectory.nativePath;
            if((_loc5_ = _loc8_.resolvePath(param3)).exists)
            {
               if(_loc5_.isDirectory)
               {
                  this._dateHeader = "Date: " + this.toRFC802(new Date());
                  this._mimeHeader = "Content-Type: text/html";
                  _loc5_.addEventListener(FileListEvent.DIRECTORY_LISTING,this.onDirectoryList);
                  _loc5_.getDirectoryListingAsync();
               }
               else
               {
                  _loc7_ = _loc5_.modificationDate;
                  this._dateHeader = "Date: " + this.toRFC802(_loc7_);
                  this._mimeHeader = this.getMimeHeader(_loc5_);
                  ...
                  this._stream.openAsync(_loc5_,FileMode.READ);
               }
            }
            else
            {
      ...
      private function onDirectoryList(param1:FileListEvent) : void
      {
         ...
         this._statusHeader = "HTTP/1.0 200 OK";
         var _loc4_:* = (_loc4_ = "<html><head><title>Dofus Module Server</title></head><body><h2>Index of /" + this.getRelativePath(File(param1.target)) + "</h2><ul>") + "<table><tr><td></td><td><b>Name</b></td><td><b>Last modified</b></td><td><b>Size (octet)</b></td></tr>";
         if(this.getRelativePath(File(param1.target)) != "")
         {
            _loc4_ = _loc4_ + ("<tr><td>" + this.getImg(File(param1.target).parent) + "</td><td><a href=\"" + HttpServer.getInstance().getUrlTo(this.getRelativePath(File(param1.target).parent)) + "\">..</a></td><td>-</td><td>-</td></tr>");
         }
         for each(_loc2_ in param1.files)
         {
            _loc3_ = _loc2_.isDirectory;
            _loc4_ = _loc4_ + ("<tr>" + "<td>" + this.getImg(_loc2_) + "</td>" + "<td><a href=\"" + HttpServer.getInstance().getUrlTo(this.getRelativePath(_loc2_)) + "\">" + _loc2_.name + (!!_loc3_?"/":"") + "</a></td>" + "<td>" + this.toRFC802(_loc2_.modificationDate) + "</td>" + "<td>" + (!!_loc3_?"-":_loc2_.size) + "</td>" + "</tr>");
         }
         _loc4_ = _loc4_ + "</table></body></html>";
         this._contentBytes = new ByteArray();
         this._contentBytes.writeUTFBytes(_loc4_);
         this._contentBytes.position = 0;
         this._contentLengthHeader = "Content-Length: " + this._contentBytes.bytesAvailable;
         this.sendResponse();
      }
