package com.ankamagames.berilia.types.graphic
{
   ...
   public class ButtonContainer extends StateContainer implements IRadioItem, FinalizableUIComponent, IDragAndDropHandler
   {
      ...
      override public function process(param1:Message) : Boolean
      {
         ...
         var _loc5_:RadioGroup = null;
         var _loc6_:GraphicContainer = null;
         var _loc7_:uint = 9999;
         ...
         if(!this._disabled)
         {
            switch(true)
            {
               case param1 is MouseDownMessage:
                  ...
                  _loc7_ = !!this._selected?uint(StatesEnum.STATE_SELECTED_CLICKED):uint(StatesEnum.STATE_CLICKED);
                  break;
               ...
               case param1 is MouseClickMessage:
                  ...
                  _loc7_ = !!this._selected?uint(StatesEnum.STATE_SELECTED_OVER):uint(StatesEnum.STATE_OVER);
                  if(!changingStateData[_loc7_])
                  {
                     _loc7_ = !!this._selected?uint(StatesEnum.STATE_SELECTED):uint(StatesEnum.STATE_NORMAL);
                  }
                  if(!this.isMute)
                  {
                     for each(_loc2_ in Berilia.getInstance().UISoundListeners)
                     {
                        _loc3_ = this.selectSound();
                        if(int(_loc3_) != -1)
                        {
                           _loc2_.playUISound(_loc3_);
                        }
                     }
                  }
                  break;
               case param1 is MouseOverMessage:
                  ...
                  else
                  {
                     if(this._playRollOverSound && !this.isMute)
                     {
                        ...
                        {
                           _loc4_.playUISound("16010");
                        }
                     }
                     _loc7_ = !!this._selected?uint(StatesEnum.STATE_SELECTED_OVER):uint(StatesEnum.STATE_OVER);
                     if(changingStateData && !changingStateData[_loc7_])
                     {
                        _loc7_ = !!this._selected?uint(StatesEnum.STATE_SELECTED):uint(StatesEnum.STATE_NORMAL);
                     }
                  }
         ...
         if(_loc7_ != 9999)
         {
            ...
            {
               if(_loc5_ = getUi().getRadioGroup(this._radioGroup))
               {
                  _loc5_.selectedItem = this;
               }
            }
         }
         if(this._sLinkedTo)
         {
            if(_loc6_ = getUi().getElement(this._sLinkedTo))
            {
               _loc6_.process(param1);
            }
         }
         return false;
      }
