   ...
   public class StateContainer extends GraphicContainer implements UIComponent
   {
      ...
      protected function makeSnapshot(param1:*, param2:GraphicContainer) : void
      {
         ...
         if(!this._snapshot[param1][param2.name])
         {
            ...
            var _loc5_:XML = this._describeType(param2);
            for each(_loc4_ in _loc5_..accessor)
            {
               if(_loc4_.@access == "readwrite")
               {
                  _loc3_ = _loc4_.@name;
                  if(!this._lockedProperties[_loc3_])
                  {
                     switch(true)
                     {
                        case param2[_loc3_] is Boolean:
                        case param2[_loc3_] is uint:
                        case param2[_loc3_] is int:
                        case param2[_loc3_] is Number:
                        case param2[_loc3_] is String:
                        case param2[_loc3_] == null:
                           this._snapshot[param1][param2.name][_loc3_] = param2[_loc3_];
                     }
                  }
               }
            }
