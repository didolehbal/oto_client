package com.ankamagames.atouin.entities.behaviours.movements
{
   ...
   public class AnimatedMovementBehavior implements IMovementBehavior
   {
      ...
      protected static function getFromCache(param1:Number, param2:Class) : AnimatedMovementBehavior
      {
         ...
         (_loc4_ = new param2() as AnimatedMovementBehavior).speedAdjust = param1;
      ...
      public function synchroniseSubEntitiesPosition(param1:IMovable, param2:DisplayObject = null) : void
      {
         ...
         if(param1 is TiphonSprite)
         {
            ...
            else if((_loc7_ = _loc3_.getSubEntitySlot(2,0) as TiphonSprite) && _loc7_.carriedEntity)
            {
      ...
      protected function goNextCell(param1:IMovable) : void
      {
         ...
         var _loc3_:TweenEntityData = _aEntitiesMoving[param1.id];
         _loc3_.currentCell = param1.position;
         ...
         if(_loc3_.path.path.length > 0)
         {
            _loc2_ = _loc3_.path.path.shift() as PathElement;
            if(this.mustChangeOrientation())
            {
               IAnimated(param1).setAnimationAndDirection(_loc3_.animation,_loc3_.orientation);
            }
            else
            {
               IAnimated(param1).setAnimation(_loc3_.animation);
            }
            _loc3_.velocity = this.getVelocity(_loc3_,_loc3_.orientation);
            _loc3_.nextCell = _loc2_.step;
            if(this.mustChangeOrientation() && _loc2_)
            {
               _loc3_.orientation = _loc2_.orientation;
            }
            _loc3_.start = getTimer();
         }
         else if(!_loc3_.currentCell.equals(_loc3_.path.end))
         {
            _loc3_.velocity = this.getVelocity(_loc3_,IAnimated(param1).getDirection());
            if(this.mustChangeOrientation() && _loc3_.orientation != -1)
            {
               IAnimated(param1).setDirection(_loc3_.orientation);
            }
            _loc3_.nextCell = _loc3_.path.end;
            _loc3_.start = getTimer();
         }
         else
         {
      ...
      {
         var _loc3_:ISoundPositionListener = null;
         ...
         (_loc5_ = DisplayObject(param1.entity)).x = (1 - param1.barycentre) * param1.currentCellSprite.x + param1.barycentre * param1.nextCellSprite.x + param1.currentCellSprite.width / 2;
         _loc5_.y = (1 - param1.barycentre) * param1.currentCellSprite.y + param1.barycentre * param1.nextCellSprite.y + param1.currentCellSprite.height / 2;
         for each(_loc3_ in Atouin.getInstance().movementListeners)
         {
            _loc4_ = new Point(_loc5_.x,_loc5_.y);
            _loc3_.setSoundSourcePosition(param1.entity.id,_loc4_);
         }
         if(!param1.wasOrdered && param1.barycentre > 0.5)
         {
            EntitiesDisplayManager.getInstance().orderEntity(_loc5_,param1.nextCellSprite);
         }
         if(param1.barycentre >= 1)
         {
            param1.clear();
      ...
      protected function processJump(param1:IMovable, param2:MapPoint) : void
      {
         ...
         (_loc4_ = param1 as DisplayObject).x = _loc3_.x + _loc3_.width / 2;
      ...
      {
         var _loc2_:TweenEntityData = null;
         var _loc3_:uint = getTimer();
         for each(_loc2_ in _aEntitiesMoving)
         {
            this.processMovement(_loc2_,_loc3_);
         }
      }
