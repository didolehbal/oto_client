package com.ankamagames.atouin.managers
{
   ...
   public class MapDisplayManager
   {
      ...
      public function fromMap(param1:Map, param2:ByteArray = null, param3:Boolean = true) : uint
      {
         ...
         var _loc5_:ResourceLoadedEvent;
         (_loc5_ = new ResourceLoadedEvent(ResourceLoadedEvent.LOADED)).resource = param1;
      ...
      public function activeIdentifiedElements(param1:Boolean) : void
      {
         ...
         var _loc3_:Dictionary = this._renderer.identifiedElements;
         for each(_loc2_ in _loc3_)
         {
            _loc2_.sprite.mouseEnabled = param1;
         }
      }
      ...
      {
         var _loc1_:Map = null;
         var _loc2_:MapsLoadingCompleteMessage = null;
         var _loc3_:Atouin = null;
         ...
         var _loc4_:RenderRequest;
         var _loc5_:WorldPoint = (_loc4_ = RenderRequest(this._renderRequestStack[0])).map;
         var _loc6_:Boolean = _loc4_.forceReloadWithoutCache;
         Atouin.getInstance().showWorld(true);
         this._renderer.initRenderContainer(Atouin.getInstance().worldContainer);
         if(!_loc6_ && this._currentMap && this._currentMap.mapId == _loc5_.mapId && !Atouin.getInstance().options.reloadLoadedMap)
         {
            this._renderRequestStack.shift();
            _log.debug("Map " + _loc5_.mapId + " is the same, renderRequestID: " + _loc4_.renderId);
            _loc1_ = MapDisplayManager.getInstance().getDataMapContainer().dataMap;
            _loc2_ = new MapsLoadingCompleteMessage(this._currentMap,_loc1_);
            _loc3_ = Atouin.getInstance();
            _loc3_.handler.process(_loc2_);
            _loc2_.renderRequestId = _loc4_.renderId;
            if(_loc1_.zoomScale != 1 && _loc3_.options.useInsideAutoZoom)
            {
               _loc3_.rootContainer.scaleX = _loc1_.zoomScale;
               _loc3_.rootContainer.scaleY = _loc1_.zoomScale;
               _loc3_.rootContainer.x = _loc1_.zoomOffsetX;
               _loc3_.rootContainer.y = _loc1_.zoomOffsetY;
               _loc3_.currentZoom = _loc1_.zoomScale;
            }
            this.checkForRender();
            return;
         }
         ...
         this._currentMap = _loc5_;
         this._currentRenderId = _loc4_.renderId;
         this._forceReloadWithoutCache = _loc6_;
         var _loc7_:MapsLoadingStartedMessage = new MapsLoadingStartedMessage();
         Atouin.getInstance().handler.process(_loc7_);
         this._nMapLoadStart = getTimer();
         this._loader.cancel();
         this._loader.load(new Uri(getMapUriFromId(_loc5_.mapId)),null);
      }
      ...
      private function mapRendered(param1:RenderMapEvent) : void
      {
         ...
         if(param1.type == RenderMapEvent.MAP_RENDER_END)
         {
            ...
            (_loc5_ = new MapLoadedMessage()).dataLoadingTime = _loc3_;
