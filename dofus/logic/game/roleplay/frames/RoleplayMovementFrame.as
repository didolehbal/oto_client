package com.ankamagames.dofus.logic.game.roleplay.frames
{
   ...
   public class RoleplayMovementFrame implements Frame
   {
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case param1 is GameMapMovementMessage:
               ...
               if((_loc5_ = _loc3_ as TiphonSprite) && !_loc4_.isCreatureMode && _loc5_.getSubEntitySlot(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER,0) && !_loc5_.getSubEntityBehavior(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER))
               {
            ...
            case param1 is EntityMovementCompleteMessage:
               if((_loc7_ = param1 as EntityMovementCompleteMessage).entity.id == PlayedCharacterManager.getInstance().id)
               {
            ...
            case param1 is EntityMovementStoppedMessage:
               if((_loc8_ = param1 as EntityMovementStoppedMessage).entity.id == PlayedCharacterManager.getInstance().id)
               {
                  (_loc12_ = new GameMapMovementCancelMessage()).initGameMapMovementCancelMessage(_loc8_.entity.position.cellId);
                  ConnectionsHandler.getConnection().send(_loc12_);
                  this._isRequestingMovement = false;
                  if(this._followingMove)
                  {
                     this.askMoveTo(this._followingMove);
                     if((_loc13_ = Kernel.getWorker().getFrame(StackManagementFrame) as StackManagementFrame).stackOutputMessage.length > 0)
                     {
                        if((_loc14_ = _loc13_.stackOutputMessage[0] as MoveBehavior) && _loc14_.position.cellId != this._followingMove.cellId)
                        {
            ...
            case param1 is TeleportOnSameMapMessage:
               ...
               if(_loc10_ = DofusEntities.getEntity(_loc9_.targetId))
               {
      ...
      function askMoveTo(param1:MapPoint) : Boolean
      {
         ...
         var _loc5_:IEntity;
         if(!(_loc5_ = DofusEntities.getEntity(PlayedCharacterManager.getInstance().id)))
         {
      ...
      {
         var _loc3_:InteractiveUseRequestMessage = null;
         var _loc4_:RoleplayInteractivesFrame;
         if((_loc4_ = Kernel.getWorker().getFrame(RoleplayInteractivesFrame) as RoleplayInteractivesFrame) && _loc4_.currentRequestedElementId != param2.elementId && !_loc4_.usingInteractive && !_loc4_.isElementChangingState(param2.elementId))
         {
            _loc4_.currentRequestedElementId = param2.elementId;
            _loc3_ = new InteractiveUseRequestMessage();
            _loc3_.initInteractiveUseRequestMessage(param2.elementId,param1);
            ConnectionsHandler.getConnection().send(_loc3_);
         }
      }
