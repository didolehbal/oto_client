package com.ankamagames.dofus.logic.game.roleplay.frames
{
   ...
   public class MonstersInfoFrame implements Frame
   {
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case param1 is EntityMouseOutMessage:
               _loc2_ = (_loc4_ = param1 as EntityMouseOutMessage).entity as AnimatedCharacter;
               if(_loc2_)
               {
      ...
      public function update(param1:Boolean = false) : void
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:int = this._roleplayEntitiesFrame.monstersIds.length;
         this.updateMouseOverMonstersIds(this._roleplayWorldFrame.mouseOverEntityId);
         this.displayMouseOverEntityTooltip(false);
         _loc2_ = 0;
         while(_loc2_ < _loc4_)
         {
            _loc3_ = this._roleplayEntitiesFrame.monstersIds[_loc2_];
            if(param1 || !TooltipManager.isVisible("MonstersInfo_" + _loc3_))
            {
               TooltipPlacer.waitBeforeOrder("tooltip_MonstersInfo_" + _loc3_);
            }
            _loc2_++;
         }
         _loc2_ = 0;
         while(_loc2_ < _loc4_)
         {
            ...
            if(param1 || !TooltipManager.isVisible("MonstersInfo_" + _loc3_))
            {
               this.showToolTip(_loc3_,"MonstersInfoCache" + _loc2_);
            }
            _loc2_++;
         }
      }
      ...
      private function updateMouseOverMonstersIds(param1:int) : void
      {
         ...
         if(_loc3_)
         {
            _loc2_ = _loc3_.getRootEntity().id;
            if(this._roleplayEntitiesFrame.getEntityInfos(_loc2_) is GameRolePlayGroupMonsterInformations)
            {
               this._mouseOverMonsterId = param1;
               this._mouseOverRootMonsterId = _loc2_;
            }
         }
         else
         {
      ...
      private function showToolTip(param1:int, param2:String = null) : void
      {
         ...
         var _loc5_:GameRolePlayGroupMonsterInformations = null;
         var _loc6_:AnimatedCharacter;
         if(_loc6_ = DofusEntities.getEntity(param1) as AnimatedCharacter)
         {
            TooltipManager.hide("entity_" + param1);
            if(_loc6_.isMoving && this._movingGroups.indexOf(_loc6_.id) == -1)
            {
               this._movingGroups.push(_loc6_.id);
            }
            if(param2)
            {
               this._tooltipsCacheNames[_loc6_.id] = param2;
            }
            _loc3_ = !!this._roleplayEntitiesFrame.hasIcon(_loc6_.id)?45:0;
            _loc4_ = _loc6_ as AnimatedCharacter;
            _loc5_ = this._roleplayEntitiesFrame.getEntityInfos(_loc6_.id) as GameRolePlayGroupMonsterInformations;
            ...
            {
               TooltipManager.show(_loc5_,_loc6_.absoluteBounds,UiModuleManager.getInstance().getModule("Ankama_Tooltips"),false,"MonstersInfo_" + _loc6_.id,LocationEnum.POINT_BOTTOM,LocationEnum.POINT_TOP,_loc3_,true,null,null,null,this._tooltipsCacheNames[_loc6_.id],false,StrataEnum.STRATA_WORLD,this._sysApi.getCurrentZoom());
            }
         }
      }
   }
