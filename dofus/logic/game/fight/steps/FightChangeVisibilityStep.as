package com.ankamagames.dofus.logic.game.fight.steps
{
   ...
   public class FightChangeVisibilityStep extends AbstractSequencable implements IFightStep
   {
      ...
      private function respawnEntity() : DisplayObject
      {
         ...
         var _loc3_:TiphonSprite = DofusEntities.getEntity(this._entityId) as TiphonSprite;
         if(_loc3_ && _loc3_.parentSprite)
         {
            _loc1_ = Kernel.getWorker().getFrame(FightEntitiesFrame) as FightEntitiesFrame;
            if(_loc1_)
            {
               _loc1_.addOrUpdateActor(_loc1_.getEntityInfos(this._entityId));
            }
            if(FightEntitiesHolder.getInstance().getEntity(this._entityId))
            {
         ...
         if(FightEntitiesHolder.getInstance().getEntity(this._entityId))
         {
            _loc2_ = DofusEntities.getEntity(this._entityId) as IDisplayable;
            _loc2_.display();
            FightEntitiesHolder.getInstance().unholdEntity(this._entityId);
         }
