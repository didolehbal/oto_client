   ...
   public class FightBattleFrame implements Frame
   {
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case param1 is GameFightTurnStartMessage:
               ...
               if(!(_loc7_ = param1 is GameFightTurnResumeMessage))
               {
               ...
               if(_loc5_.id > 0 || this._playingSlaveEntity)
               {
                  ...
                  {
                     if((_loc29_ = DofusEntities.getEntity(_loc5_.id) as AnimatedCharacter) != null)
                     {
                        (_loc30_ = new SerialSequencer()).addStep(new AddGfxEntityStep(154,_loc29_.position.cellId));
                        _loc30_.start();
                        _loc31_ = 65 * _loc29_.look.getScaleY();
                        (_loc32_ = new SerialSequencer()).addStep(new AddGfxEntityStep(153,_loc29_.position.cellId,0,-_loc31_));
                        _loc32_.start();
                     }
               ...
               KernelEventsManager.getInstance().processCallback(HookList.GameFightTurnStart,_loc5_.id,_loc5_.waitTime * 100,!!_loc7_?(param1 as GameFightTurnResumeMessage).remainingTime:_loc5_.waitTime * 100,Dofus.getInstance().options.turnPicture);
               ...
               _loc11_ = (_loc10_ = FightEntitiesFrame.getCurrentInstance().getEntityInfos(FightContextFrame.fighterEntityTooltipId) as GameFightFighterInformations) && _loc10_.disposition.cellId == FightContextFrame.currentCell;
               ...
               if(_loc14_ = (_loc13_ = Kernel.getWorker().getFrame(FightContextFrame) as FightContextFrame).fightersPositionsHistory[_loc5_.id])
               {
                  _loc40_ = _loc14_.length;
                  _loc39_ = 0;
                  while(_loc39_ < _loc40_)
                  {
                     --(_loc38_ = _loc14_[_loc39_]).lives;
                     if(_loc38_.lives == 0)
                     {
            ...
            case param1 is GameFightTurnEndMessage:
               ...
               if((_loc16_ = FightEntitiesFrame.getCurrentInstance().getEntityInfos(_loc15_.id)) is GameFightFighterInformations && !(_loc16_ as GameFightFighterInformations).alive)
               {
            ...
            case param1 is GameFightEndMessage:
               ...
               while(this._currentSequenceFrame && --_loc25_)
               {
                  ...
                  (_loc43_ = new SequenceEndMessage()).initSequenceEndMessage();
            ...
            case param1 is GameContextDestroyMessage:
               ...
               else
               {
                  ...
                  (_loc44_ = new GameFightEndMessage()).initGameFightEndMessage(0,0,0,null);
      ...
      public function prepareNextPlayableCharacter(param1:int = 0) : void
      {
         ...
         if(this._slaveId)
         {
            if(param1)
            {
               _loc3_ = param1 == this._slaveId?int(this._masterId):int(this._slaveId);
            }
            else
            {
      ...
      public function getNextPlayableCharacterId() : int
      {
         ...
         if(_loc1_ < _loc3_ && _loc2_ > _loc3_)
         {
            return this._slaveId;
         }
         if(_loc1_ > _loc3_ && _loc2_ < _loc3_)
      ...
      private function confirmTurnEnd() : void
      {
         ...
         var _loc2_:int = 0;
         var _loc3_:GameFightFighterInformations = FightEntitiesFrame.getCurrentInstance().getEntityInfos(this._lastPlayerId) as GameFightFighterInformations;
         if(_loc3_)
         {
            BuffManager.getInstance().markFinishingBuffs(this._lastPlayerId);
            _loc3_.stats.actionPoints = _loc3_.stats.maxActionPoints;
            _loc3_.stats.movementPoints = _loc3_.stats.maxMovementPoints;
            if(this._lastPlayerId == CurrentPlayedFighterManager.getInstance().currentFighterId)
            {
               _loc1_ = CurrentPlayedFighterManager.getInstance().getCharacteristicsInformations();
               _loc1_.actionPointsCurrent = _loc3_.stats.maxActionPoints;
               _loc1_.movementPointsCurrent = _loc3_.stats.maxMovementPoints;
               KernelEventsManager.getInstance().processCallback(HookList.CharacterStatsList);
               CurrentPlayedFighterManager.getInstance().getSpellCastManager().nextTurn();
               SpellWrapper.refreshAllPlayerSpellHolder(this._lastPlayerId);
               for each(_loc2_ in this._playerTargetedEntitiesList)
               {
                  TooltipManager.hide("tooltip_tooltipOverEntity_" + _loc2_);
               }
         ...
         var _loc4_:GameFightTurnReadyMessage;
         (_loc4_ = new GameFightTurnReadyMessage()).initGameFightTurnReadyMessage(true);
      ...
      private function endBattle(param1:GameFightEndMessage) : void
      {
         ...
         var _loc4_:FightEntitiesHolder;
         var _loc5_:Dictionary = (_loc4_ = FightEntitiesHolder.getInstance()).getEntities();
         for each(_loc2_ in _loc5_)
         {
            (_loc2_ as AnimatedCharacter).display();
         }
         _loc4_.reset();
         this._synchroniseFighters = null;
         Kernel.getWorker().removeFrame(this);
         _loc3_ = Kernel.getWorker().getFrame(FightContextFrame) as FightContextFrame;
         _loc3_.process(param1);
      }
      ...
      private function gameFightSynchronize(param1:Vector.<GameFightFighterInformations>, param2:Boolean = true) : void
      {
         ...
         var _loc10_:FightBattleFrame = Kernel.getWorker().getFrame(FightBattleFrame) as FightBattleFrame;
         var _loc11_:BuffManager = BuffManager.getInstance();
         if(param2)
         {
            _loc7_ = 0;
            if(this.currentPlayerId == this.masterId || this.currentPlayerId == this.slaveId)
            {
               _loc7_ = this.currentPlayerId == this.slaveId?int(this.masterId):int(this.slaveId);
            }
            BuffManager.getInstance().synchronize(_loc7_);
         }
         for each(_loc6_ in param1)
         {
            if(_loc6_.alive)
            {
               ...
               _loc4_ = _loc6_.wave == this._newWaveId && !DofusEntities.getEntity(_loc6_.contextualId);
               _loc9_.updateFighter(_loc6_,null,BuffManager.getInstance().getFinishingBuffs(_loc6_.contextualId));
               if(_loc4_)
               {
                  _loc3_ = true;
                  (DofusEntities.getEntity(_loc6_.contextualId) as AnimatedCharacter).visible = false;
                  if(!OptionManager.getOptionManager("tubul")["tubulIsDesactivated"] && _loc5_ == 0)
                  {
                     this._waveSound.play();
                  }
                  (_loc8_ = new SerialSequencer()).addStep(new WaitStep(300 * _loc5_));
                  _loc8_.addStep(new AddGfxEntityStep(2715,_loc6_.disposition.cellId));
                  _loc8_.addStep(new FightVisibilityStep(_loc6_.contextualId,true));
                  _loc8_.start();
                  _loc5_++;
               }
            }
