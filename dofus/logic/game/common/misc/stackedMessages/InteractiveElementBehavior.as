package com.ankamagames.dofus.logic.game.common.misc.stackedMessages
{
   ...
   public class InteractiveElementBehavior extends AbstractBehavior
   {
      ...
      {
         var _loc3_:InteractiveElementActivationMessage = null;
         ...
         canBeStacked = true;
         var _loc8_:IEntity = DofusEntities.getEntity(PlayedCharacterManager.getInstance().id);
         ...
         {
            _loc3_ = param1 as InteractiveElementActivationMessage;
            type = ALWAYS;
            if(_loc4_ = Interactive.getInteractiveById(_loc3_.interactiveElement.elementTypeId))
            {
               for each(_loc6_ in _loc3_.interactiveElement.enabledSkills)
               {
                  if((_loc5_ = Skill.getSkillById(_loc6_.skillId)).elementActionId == ACTION_CLASS_STATUE || _loc5_.elementActionId == ACTION_WAYPOINT)
                  {
                     type = STOP;
                  }
               }
            ...
            this.interactiveElement = _loc3_.interactiveElement;
            this.currentSkillInstanceId = _loc3_.skillInstanceId;
            position = _loc3_.position;
            interactiveElements[_loc3_.interactiveElement.elementId] = true;
            InteractiveElementBehavior.currentElementId = _loc3_.interactiveElement.elementId;
            pendingMessage = param1;
            _loc9_ = true;
         }
         else if(param1 is GameMapMovementMessage && (param1 as GameMapMovementMessage).actorId == PlayedCharacterManager.getInstance().id && (param1 as GameMapMovementMessage).keyMovements[0] == _loc8_.position.cellId)
         {
            this._isMoving = true;
         ...
         {
            if((_loc7_ = param1 as InteractiveElementUpdatedMessage).interactiveElement.enabledSkills.length)
            {
