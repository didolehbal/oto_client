package com.ankamagames.dofus.logic.game.common.misc.inventoryView
{
   ...
   public class StorageGenericView implements IStorageView
   {
      ...
      public function modifyItem(param1:ItemWrapper, param2:ItemWrapper, param3:int) : void
      {
         ...
         var _loc5_:int;
         if((_loc5_ = this.getItemIndex(param1)) != -1)
         {
            if((_loc4_ = this._content[_loc5_]).quantity == param1.quantity - param3)
            {
               _loc4_.update(param1.position,param1.objectUID,param1.objectGID,_loc4_.quantity,param1.effectsList);
               this.updateView();
            }
            else if(param1.quantity <= param3)
            {
               this.removeItem(_loc4_,param3);
            }
            else
            {
               _loc4_.update(param1.position,param1.objectUID,param1.objectGID,param1.quantity - param3,param1.effectsList);
               this.updateView();
            }
         }
      ...
      protected function getItemIndex(param1:ItemWrapper, param2:Vector.<ItemWrapper> = null) : int
      {
         ...
         while(_loc4_ < param2.length)
         {
            ...
            if(_loc3_.objectUID == param1.objectUID)
            {
               return _loc4_;
            }
            _loc4_++;
      ...
      public function updateView() : void
      {
         ...
         {
            _loc2_ = this._sortFieldsCache.length;
            _loc1_ = 0;
            while(_loc1_ < _loc2_)
            {
               if(this._sortFieldsCache[_loc1_] != this.sortFields()[_loc1_])
               {
                  _loc4_ = false;
                  break;
               }
         ...
         if(this._sortFieldsCache[0] != StorageOptionManager.SORT_FIELD_NONE)
         {
            ...
            if(!this._sortedContent)
            {
               ...
               for each(_loc3_ in this._content)
               {
                  this._sortedContent.push(_loc3_);
               }
            }
