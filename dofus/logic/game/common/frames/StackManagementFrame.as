package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class StackManagementFrame implements Frame
   {
      ...
      {
         var _loc2_:RemoveBehaviorToStackAction = null;
         var _loc3_:uint = param1 == null?uint(KEY_CODE):uint(param1.keyCode);
         if(this._keyDown && _loc3_ == KEY_CODE)
         {
            this._keyDown = false;
            _loc2_ = new RemoveBehaviorToStackAction();
            _loc2_.behavior = StackActionEnum.REMOVE_ALL;
            Kernel.getWorker().process(_loc2_);
         }
      }
      ...
      private function getInputMessageAlreadyWatched(param1:Vector.<AbstractBehavior>, param2:Class) : AbstractBehavior
      {
         ...
         {
            if(getQualifiedClassName(_loc3_) == _loc4_)
            {
               return _loc3_;
            }
         }
      ...
      private function initStackInputMessages(param1:String) : void
      {
         ...
         var _loc5_:Vector.<AbstractBehavior> = new Vector.<AbstractBehavior>();
         if(this._stackInputMessage != null)
         {
            _loc5_ = this._stackInputMessage.concat();
         }
         this._stackInputMessage = new Vector.<AbstractBehavior>();
         for each(_loc2_ in BEHAVIOR_LIST)
         {
            _loc3_ = new _loc2_();
            if(param1 == AbstractBehavior.NORMAL || param1 == AbstractBehavior.ALWAYS && _loc3_.type == AbstractBehavior.ALWAYS)
            {
               if((_loc4_ = this.getInputMessageAlreadyWatched(_loc5_,_loc2_)) != null)
               {
                  this._stackInputMessage.push(_loc4_);
               }
               else
               {
                  this.addBehaviorToInputStack(_loc3_,true);
               }
            }
            else
            {
               this.addBehaviorToInputStack(_loc3_,false);
            }
         }
      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case param1 is EmptyStackAction:
               ...
               return true;
            default:
               _loc7_ = (_loc6_ = getQualifiedClassName(param1).split("::")[1]) == "InteractiveUsedMessage" || _loc6_ == "InteractiveUseErrorMessage";
                     ...
                     switch(true)
                     {
                        ...
                        case _loc11_ is InteractiveElementBehavior:
                           ...
                           if(_loc7_ && _loc14_.interactiveElement.elementId == (param1 as Object).elemId)
                           {
                              ...
                              if((_loc16_ = this._waitingMessage as InteractiveElementActivationMessage) && _loc16_.interactiveElement.elementId == _loc14_.interactiveElement.elementId)
                              {
      ...
      private function processStackInputMessages(param1:Message) : Boolean
      {
         ...
         for each(_loc2_ in this._stackInputMessage)
         {
            if(_loc2_.processInputMessage(param1,this._currentMode))
            {
               ...
               {
                  _loc3_ = _loc2_.copy();
                  if((_loc4_ = this.getSameInOutputList(_loc3_)) == null)
                  {
      ...
      {
         var _loc2_:AbstractBehavior = null;
         var _loc3_:CheckPointEntity = null;
         ...
         var _loc4_:Vector.<AbstractBehavior> = this._stackOutputMessage.concat();
         for each(_loc2_ in _loc4_)
         {
            if(param1 || _loc4_.indexOf(_loc2_) != 0 || _loc4_.indexOf(_loc2_) == 0 && !_loc2_.actionStarted)
            {
               _loc2_.removeIcon();
               _loc2_.remove();
               this._stackOutputMessage.splice(this._stackOutputMessage.indexOf(_loc2_),1);
         ...
         _loc4_ = null;
         for each(_loc3_ in this._checkPointList)
         {
            EntitiesDisplayManager.getInstance().removeEntity(_loc3_);
         }
