package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class PlayedCharacterUpdatesFrame implements Frame
   {
      ...
      public function updateCharacterStatsList(param1:CharacterStatsListMessage) : void
      {
         ...
         var _loc6_:CharacterCharacteristicsInformations;
         if((_loc6_ = PlayedCharacterManager.getInstance().characteristics) && _loc6_.energyPoints > param1.stats.energyPoints)
         {
         ...
         var _loc7_:Vector.<CharacterSpellModification> = !!_loc6_?PlayedCharacterManager.getInstance().characteristics.spellModifications:null;
         var _loc8_:Vector.<CharacterSpellModification> = param1.stats.spellModifications;
         var _loc9_:int = !!_loc7_?int(Math.max(_loc7_.length,_loc8_.length)):int(_loc8_.length);
         ...
         if(_loc7_)
         {
            ...
            while(_loc2_ < _loc9_)
            {
               if(_loc7_.length <= _loc2_)
               {
                  if(_loc10_.indexOf(_loc8_[_loc2_].spellId) == -1)
                  {
                     _loc10_.push(_loc8_[_loc2_].spellId);
                  }
               }
               else if(_loc8_.length <= _loc2_)
               {
                  if(_loc10_.indexOf(_loc7_[_loc2_].spellId) == -1)
                  {
                     _loc10_.push(_loc7_[_loc2_].spellId);
                  }
               }
               else if(_loc7_[_loc2_] != _loc8_[_loc2_])
               {
                  if(_loc10_.indexOf(_loc8_[_loc2_].spellId) == -1)
                  {
                     _loc10_.push(_loc8_[_loc2_].spellId);
         ...
         {
            _loc4_ = SpellWrapper.getSpellWrappersById(_loc3_,PlayedCharacterManager.getInstance().id);
            for each(_loc5_ in _loc4_)
            {
               if(_loc5_)
               {
                  ++(_loc5_ = SpellWrapper.create(_loc5_.position,_loc5_.spellId,_loc5_.spellLevel,true,PlayedCharacterManager.getInstance().id)).versionNum;
               }
            }
         }
      ...
      {
         var _loc2_:PlayerSetInfo = null;
         var _loc3_:Vector.<uint> = null;
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc7_:int = 0;
         var _loc6_:int = this.setList.length;
         while(_loc7_ < _loc6_)
         {
            _loc2_ = this.setList[_loc7_];
            if(_loc2_)
            {
               _loc3_ = _loc2_.setObjects;
