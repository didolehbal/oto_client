      ...
      public function process(param1:Message) : Boolean
      {
         ...
         switch(true)
         {
            ...
            case param1 is EmoteAddMessage:
               ...
               if(!(_loc7_ = Emoticon.getEmoticonById(_loc5_.emoteId)))
               {
                  return true;
               }
            ...
            case param1 is EmotePlayRequestAction:
               ...
               if(!(_loc13_ = Emoticon.getEmoticonById(_loc12_.emoteId)))
               {
                  return true;
               }
               if(!(_loc14_ = EmoteWrapper.getEmoteWrapperById(_loc13_.id)) || _loc14_ && _loc14_.timer > 0)
               {
                  return true;
               }
               EmoteWrapper.getEmoteWrapperById(_loc13_.id).timerToStart = _loc13_.cooldown;
               (_loc15_ = new EmotePlayRequestMessage()).initEmotePlayRequestMessage(_loc12_.emoteId);
               if(!(_loc16_ = DofusEntities.getEntity(PlayedCharacterManager.getInstance().id)))
               {
                  return true;
               }
            ...
            case param1 is EmotePlayMessage:
               ...
               else
               {
                  ...
                  if(!(_loc30_ = Emoticon.getEmoticonById(_loc17_.emoteId)))
                  {
                     _log.error("ERREUR : Le client n\'a pas de données pour l\'emote [" + _loc17_.emoteId + "].");
                     return true;
                  }
                  _loc32_ = (_loc31_ = EntityLookAdapter.fromNetwork(_loc18_.look)).getSubEntity(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER,0);
