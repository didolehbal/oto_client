package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class PartyManagementFrame implements Frame
   {
      ...
      private function createPartyFightNotification(param1:uint, param2:PartyFightInformationsData, param3:int) : void
      {
         ...
         var _loc5_:Array;
         (_loc5_ = new Array()).push(param2.memberName);
      ...
      {
         var _loc3_:ActorAlignmentInformations = null;
         var _loc4_:PartyMemberWrapper = null;
         var _loc5_:ActorRestrictionsInformations = null;
         var _loc6_:int = 0;
         var _loc9_:Boolean = false;
         var _loc7_:* = "";
         var _loc8_:SocialApi = new SocialApi();
         if(param2 == this._arenaPartyId)
         {
            for each(_loc4_ in this._arenaPartyMembers)
            {
               if(_loc4_.id == param1)
               {
                  _loc7_ = _loc4_.name;
         ...
         {
            for each(_loc4_ in this._partyMembers)
            {
               if(_loc4_.id == param1)
               {
                  _loc7_ = _loc4_.name;
         ...
         if(this.roleplayEntitiesFrame)
         {
            for each(_loc6_ in this.roleplayEntitiesFrame.playersId)
            {
               if(_loc6_ == param1)
               {
                  _loc9_ = true;
                  if(this.roleplayEntitiesFrame.getEntityInfos(_loc6_) is GameRolePlayCharacterInformations)
                  {
                     _loc3_ = (this.roleplayEntitiesFrame.getEntityInfos(_loc6_) as GameRolePlayCharacterInformations).alignmentInfos;
                     _loc5_ = (this.roleplayEntitiesFrame.getEntityInfos(_loc6_) as GameRolePlayCharacterInformations).humanoidInfo.restrictions;
                  }
               }
            }
         }
         return MenusFactory.create({
            ...
            "name":_loc7_,
            "onSameMap":_loc9_,
            "alignmentInfos":_loc3_,
            "cantBeChallenged":_loc5_ && _loc5_.cantChallenge,
            "cantExchange":_loc5_ && _loc5_.cantExchange
         },"partyMember",param2);
      }
      ...
      private function onTimerTick(param1:TimerEvent) : void
      {
         ...
         for each(_loc2_ in this._partyMembers)
         {
            if(_loc2_.lifePoints < _loc2_.maxLifePoints && _loc2_.regenRate > 0)
            {
               ...
               {
                  (_loc6_ = new LifePointTickManager()).originalLifePoint = _loc2_.lifePoints;
               ...
               if((_loc5_ = _loc3_.originalLifePoint + _loc4_) >= _loc2_.maxLifePoints)
               {
         ...
         for each(_loc2_ in this._arenaPartyMembers)
         {
            if(_loc2_.lifePoints < _loc2_.maxLifePoints && _loc2_.regenRate > 0)
            {
               ...
               {
                  (_loc10_ = new LifePointTickManager()).originalLifePoint = _loc2_.lifePoints;
               ...
               if((_loc9_ = _loc7_.originalLifePoint + _loc8_) >= _loc2_.maxLifePoints)
               {
