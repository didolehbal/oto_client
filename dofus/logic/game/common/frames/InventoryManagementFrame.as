package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class InventoryManagementFrame implements Frame
   {
      ...
      public function getWrappersFromShortcuts(param1:Vector.<Shortcut>) : Array
      {
         ...
         {
            _loc2_ = this.getShortcutWrapperPropFromShortcut(_loc3_);
            _loc4_[_loc3_.slot] = ShortcutWrapper.create(_loc3_.slot,_loc2_.id,_loc2_.type,_loc2_.gid);
         }
         return _loc4_;
      }
      ...
      private function onCellPointed(param1:Boolean, param2:uint, param3:int) : void
      {
         ...
         if(param1)
         {
            if(param3 < 0)
            {
               (_loc4_ = new ObjectUseOnCellMessage()).initObjectUseOnCellMessage(this._currentPointUseUIDObject,param2);
               ConnectionsHandler.getConnection().send(_loc4_);
            }
            else
            {
               (_loc5_ = new ObjectUseOnCharacterMessage()).initObjectUseOnCharacterMessage(this._currentPointUseUIDObject,param3);
      ...
      private function useItem(param1:ObjectUseAction, param2:ItemWrapper) : void
      {
         ...
         {
            (_loc4_ = new ObjectUseMultipleMessage()).initObjectUseMultipleMessage(param1.objectUID,param1.quantity);
            ConnectionsHandler.getConnection().send(_loc4_);
         }
         else
         {
            (_loc5_ = new ObjectUseMessage()).initObjectUseMessage(param1.objectUID);
            ConnectionsHandler.getConnection().send(_loc5_);
         }
      }
      ...
      private function getShortcutWrapperPropFromShortcut(param1:Shortcut) : Object
      {
         ...
         if(param1 is ShortcutObjectItem)
         {
            ...
            _loc4_ = (param1 as ShortcutObjectItem).itemGID;
            _loc3_ = 0;
         }
