package com.ankamagames.dofus.externalnotification
{
   ...
   public class ExternalNotificationManager
   {
      ...
      public function getNotificationOptions(param1:int) : Object
      {
         ...
         var _loc4_:Object = StoreDataManager.getInstance().getData(this._dataStoreType,"notificationsEvent" + param1);
         var _loc5_:Boolean = this.hasNotificationData(param1);
         var _loc6_:Boolean = _loc4_ && _loc5_ && !_loc4_.hasOwnProperty("active") && !_loc4_.hasOwnProperty("sound") && !_loc4_.hasOwnProperty("multi") && !_loc4_.hasOwnProperty("notify");
         if(!_loc4_ || _loc6_)
         {
            _loc2_ = ExternalNotification.getExternalNotifications();
            _loc4_ = new Object();
            if(_loc5_)
            {
               for each(_loc3_ in _loc2_)
               {
                  ...
                  {
                     _loc4_.active = _loc3_.defaultEnable;
                     _loc4_.sound = _loc3_.defaultSound;
                     _loc4_.notify = _loc3_.defaultNotify;
                     _loc4_.multi = _loc3_.defaultMultiAccount;
                     break;
                  }
               }
            }
            ...
            this.setNotificationOptions(param1,_loc4_);
         }
         return _loc4_;
      }
      ...
      private function getExternalNotification(param1:String, param2:String) : ExternalNotificationWindow
      {
         ...
         if(this._notificationsList.length > 0)
         {
            ...
            {
               if(_loc3_.clientId == param1 && _loc3_.id == param2)
               {
                  _loc4_ = _loc3_;
                  break;
               }
      ...
      {
         var _loc2_:ExternalNotification = null;
         var _loc3_:Array = ExternalNotification.getExternalNotifications();
         for each(_loc2_ in _loc3_)
         {
            if(ExternalNotificationTypeEnum[_loc2_.name] == param1)
            {
               return true;
            }
         }
      ...
      {
         var _loc2_:ExternalNotificationWindow = null;
         this.updateDofusFocus(param1,false);
         this._slavesIds.splice(this._slavesIds.indexOf(param1),1);
         this.sendToSlaves("updateClientsIds",this._slavesIds);
         var _loc3_:Vector.<ExternalNotificationWindow> = this.getExternalNotifications(param1);
         if(_loc3_)
         {
            for each(_loc2_ in _loc3_)
            {
               this.destroyExternalNotification(_loc2_);
            }
         }
      }
      ...
      public function handleFocusRequest(param1:String, param2:String = null, param3:Array = null) : void
      {
         ...
         else
         {
            ...
            if(param2 && param3)
            {
               if(_loc4_ = Hook.getHookByName(param2))
               {
      ...
      private function setNotificationCoordinates(param1:ExternalNotificationWindow) : void
      {
         ...
         var _loc4_:int = this._notificationsList.indexOf(param1);
         switch(this._notificationsPosition)
         {
            case ExternalNotificationPositionEnum.BOTTOM_RIGHT:
               _loc2_ = Capabilities.screenResolutionX - param1.contentWidth - this._startCoordinatesX;
               if(_loc4_ == 0)
               {
                  _loc3_ = Capabilities.screenResolutionY - param1.contentHeight - this._startCoordinatesY;
               }
               break;
            case ExternalNotificationPositionEnum.BOTTOM_LEFT:
               _loc2_ = this._startCoordinatesX;
               if(_loc4_ == 0)
               {
                  _loc3_ = Capabilities.screenResolutionY - param1.contentHeight - this._startCoordinatesY;
               }
            ...
            case ExternalNotificationPositionEnum.TOP_RIGHT:
               ...
               if(_loc4_ == 0)
               {
                  _loc3_ = this._startCoordinatesY;
               }
               break;
         ...
         if(_loc4_ > 0)
         {
            ...
            {
               _loc3_ = this._notificationsList[_loc4_ - 1].y + (this._notificationsList[_loc4_ - 1].height + this.NOTIFICATION_SPACING);
            }
            else
            {
               _loc3_ = this._notificationsList[_loc4_ - 1].y - (param1.contentHeight + this.NOTIFICATION_SPACING);
            }
         }
         param1.bounds = new Rectangle(_loc2_,_loc3_,param1.contentWidth,param1.contentHeight);
      }
      ...
      private function destroyExternalNotification(param1:ExternalNotificationWindow, param2:Boolean = true) : void
      {
         ...
         param1.destroy();
         var _loc5_:int = this._notificationsList.length;
         var _loc6_:int = this._notificationsList.indexOf(param1);
         if(param2)
         {
            if(this._notificationsList.length > 0 && _loc6_ != _loc5_ - 1)
            {
               _loc4_ = param1.height + this.NOTIFICATION_SPACING;
               if(this.isTopPosition(this._notificationsPosition))
               {
                  _loc4_ = -_loc4_;
               }
               ...
               while(_loc3_ > _loc6_)
               {
                  this._notificationsList[_loc3_].y = this._notificationsList[_loc3_].y + _loc4_;
                  _loc3_--;
               }
            }
         }
         this._notificationsList.splice(_loc6_,1);
      }
   }
}
