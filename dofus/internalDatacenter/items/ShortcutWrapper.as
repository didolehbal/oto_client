package com.ankamagames.dofus.internalDatacenter.items
{
   ...
   public class ShortcutWrapper extends Proxy implements ISlotData, IDataCenter
   {
      ...
      public static function create(param1:uint, param2:uint, param3:uint = 0, param4:uint = 0) : ShortcutWrapper
      {
         ...
         var _loc7_:EmoticonFrame = null;
         var _loc8_:ShortcutWrapper;
         (_loc8_ = new ShortcutWrapper()).slot = param1;
         _loc8_.id = param2;
         _loc8_.type = param3;
         _loc8_.gid = param4;
         if(param3 == TYPE_ITEM_WRAPPER)
         {
            if(param2 != 0)
            {
               _loc5_ = InventoryManager.getInstance().inventory.getItem(param2);
            }
            else
            {
               _loc5_ = ItemWrapper.create(0,0,param4,0,null,false);
            }
            if(_loc5_)
            {
               _loc8_.quantity = _loc5_.quantity;
            }
            if(_loc8_.quantity == 0)
            {
               _loc8_.active = false;
            }
            else
         ...
         {
            _loc6_ = EmoteWrapper.create(_loc8_.id,_loc8_.slot);
            if((_loc7_ = Kernel.getWorker().getFrame(EmoticonFrame) as EmoticonFrame).isKnownEmote(_loc8_.id))
            {
               _loc8_.active = true;
            }
            else
      ...
      public function update(param1:uint, param2:uint, param3:uint = 0, param4:uint = 0) : void
      {
         ...
         if(param3 == TYPE_EMOTE_WRAPPER)
         {
            if(!(_loc6_ = Kernel.getWorker().getFrame(EmoticonFrame) as EmoticonFrame).isKnownEmote(param2))
            {
               this.active = false;
            }
         }
      ...
      public function getIconUri(param1:Boolean = true) : Uri
      {
         ...
         {
            if(_loc4_ = InventoryManager.getInstance().presets[this.id])
            {
         ...
         {
            if(_loc5_ = SpellWrapper.getFirstSpellWrapperById(this.id,this.getCharaId()))
            {
         ...
         {
            if(_loc6_ = SmileyWrapper.getSmileyWrapperById(this.id))
            {
         ...
         {
            if(_loc7_ = EmoteWrapper.getEmoteWrapperById(this.id))
            {
      ...
      public function addHolder(param1:ISlotDataHolder) : void
      {
         ...
         {
            if(_loc4_ = EmoteWrapper.getEmoteWrapperById(this.id))
            {
