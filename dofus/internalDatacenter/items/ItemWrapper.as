package com.ankamagames.dofus.internalDatacenter.items
{
   ...
   public class ItemWrapper extends Item implements ISlotData, ICellZoneProvider, IDataCenter
   {
      ...
      public static function create(param1:uint, param2:uint, param3:uint, param4:uint, param5:Vector.<ObjectEffect>, param6:Boolean = true) : ItemWrapper
      {
         ...
         var _loc9_:ItemWrapper;
         if(!(_loc9_ = param2 > 0?_cache[param2]:_cacheGId[param3]) || !param6)
         {
            if(_loc8_.isWeapon)
            {
      ...
      public function get spellZoneEffects() : Vector.<IZoneShape>
      {
         ...
         var _loc3_:Vector.<IZoneShape> = new Vector.<IZoneShape>();
         for each(_loc1_ in this.effects)
         {
      ...
      public function get favoriteEffect() : Vector.<EffectInstance>
      {
         ...
         var _loc5_:Vector.<EffectInstance> = new Vector.<EffectInstance>();
         if(PlayedCharacterManager.getInstance())
         {
            _loc1_ = PlayedCharacterManager.getInstance().currentSubArea;
            _loc2_ = Item.getItemById(this.objectGID);
            if(_loc1_ && _loc2_.favoriteSubAreas.indexOf(_loc1_.id) != -1)
            {
               if(_loc2_.favoriteSubAreas && _loc2_.favoriteSubAreas.length && _loc2_.favoriteSubAreasBonus)
               {
                  for each(_loc4_ in this.effects)
                  {
                     if(_loc4_ is EffectInstanceInteger && _loc4_.bonusType == 1)
                     {
                        _loc3_ = _loc4_.clone();
                        EffectInstanceInteger(_loc3_).value = Math.floor(EffectInstanceInteger(_loc3_).value * _loc2_.favoriteSubAreasBonus / 100);
                        if(EffectInstanceInteger(_loc3_).value)
                        {
                           _loc5_.push(_loc3_);
                        }
                     }
         ...
         if(!this._shortName)
         {
            switch(this.objectGID)
            {
               case OBJECT_GID_SOULSTONE:
                  ...
                  {
                     if(_loc6_ = Monster.getMonsterById(int(_loc5_.parameter2)))
                     {
                        if((_loc7_ = int(_loc5_.parameter0)) < 1 || _loc7_ > _loc6_.grades.length)
                        {
                           _loc7_ = _loc6_.grades.length;
                        }
                        if((_loc8_ = _loc6_.grades[_loc7_ - 1]) && _loc8_.level > _loc1_)
                        {
                           _loc1_ = _loc8_.level;
                           _loc2_ = _loc6_.name;
                        }
               ...
               case OBJECT_GID_SOULSTONE_MINIBOSS:
                  ...
                  for each(_loc5_ in this.effects)
                  {
                     if((_loc6_ = Monster.getMonsterById(int(_loc5_.parameter2))) && _loc6_.isMiniBoss)
                     {
               ...
               case OBJECT_GID_SOULSTONE_BOSS:
                  ...
                  for each(_loc5_ in this.effects)
                  {
                     if((_loc6_ = Monster.getMonsterById(int(_loc5_.parameter2))) && _loc6_.isBoss)
                     {
      ...
      public function getIconUri(param1:Boolean = true) : Uri
      {
         ...
         var _loc5_:Item = Item.getItemById(this.objectGID);
         ...
         if(this.isLivingObject)
         {
            _loc2_ = LivingObjectSkinJntMood.getLivingObjectSkin(!!this.livingObjectId?int(this.livingObjectId):int(this.objectGID),this.livingObjectMood,this.livingObjectSkin).toString();
         }
         else if(this.isObjectWrapped)
         {
            _loc3_ = Item.getItemById(this._wrapperItemSkinGID);
            _loc2_ = !!_loc3_?_loc3_.iconId.toString():"error_on_item_" + this.objectGID + ".png";
         }
         else if(this.isMimicryObject)
         {
            _loc2_ = !!(_loc4_ = Item.getItemById(this._mimicryItemSkinGID))?_loc4_.iconId.toString():"error_on_item_" + this.objectGID + ".png";
         }
         else
         {
            _loc2_ = !!_loc5_?_loc5_.iconId.toString():"error_on_item_" + this.objectGID + ".png";
         }
         if(param1)
         {
            this._uriPngMode = new Uri(XmlConfig.getInstance().getEntry("config.gfx.path.item.bitmap").concat(_loc2_).concat(".png"));
            return this._uriPngMode;
         }
         this._uri = new Uri(XmlConfig.getInstance().getEntry("config.gfx.path.item.vector").concat(_loc2_).concat(".swf"));
         if(!_uriLoaderContext)
         {
            _uriLoaderContext = new LoaderContext();
            AirScanner.allowByteCodeExecution(_uriLoaderContext,true);
      ...
      {
         var _loc2_:ObjectEffect = null;
         var _loc3_:EffectInstance = null;
         var _loc5_:uint = 0;
         var _loc6_:uint = 0;
         var _loc4_:Item;
         if((_loc4_ = Item.getItemById(this.objectGID)) && _loc4_.isWeapon)
         {
            switch(_loc4_.typeId)
            {
               case 7:
                  _loc5_ = 88;
                  _loc6_ = 1;
         ...
         {
            _loc3_ = ObjectEffectAdapter.fromNetwork(_loc2_);
            if(_loc5_ && _loc3_.category == 2)
            {
               _loc3_.zoneShape = _loc5_;
               _loc3_.zoneSize = _loc6_;
            }
            this.effects.push(_loc3_);
            this.updateLivingObjects(_loc3_);
            this.updatePresets(_loc3_);
            if(_loc3_.effectId == 139 || _loc3_.effectId == 110)
            {
               this.isOkForMultiUse = true;
            }
            if(_loc3_.effectId == 983)
            {
               this.exchangeAllowed = false;
            }
            if(_loc3_.effectId == 981 || _loc3_.effectId == 982)
            {
               exchangeable = false;
            }
            if(_loc3_.effectId == ACTION_ID_WRAPPER_OBJECT_CATEGORY)
            {
               this.wrapperObjectCategory = EffectInstanceInteger(_loc3_).value;
            }
         }
      }
   }
