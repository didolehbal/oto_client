package com.ankamagames.dofus.internalDatacenter.items
{
   ...
   public class LivingObjectSkinWrapper implements ISlotData
   {
      ...
      public function getIconUri(param1:Boolean = true) : Uri
      {
         ...
         if(_loc3_)
         {
            _loc2_ = LivingObjectSkinJntMood.getLivingObjectSkin(this._id,this._mood,this._skin);
            if(param1)
            {
               this._pngMode = true;
               this._uri = new Uri(XmlConfig.getInstance().getEntry("config.gfx.path.item.bitmap").concat(_loc2_).concat(".png"));
            }
            else
            {
               this._pngMode = false;
