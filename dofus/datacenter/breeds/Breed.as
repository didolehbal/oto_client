package com.ankamagames.dofus.datacenter.breeds
{
   ...
   public class Breed implements IDataCenter
   {
      ...
      public static function getBreedFromSkin(param1:int) : Breed
      {
         ...
         {
            for each(_loc3_ in getBreeds())
            {
               _loc4_ = (_loc4_ = _loc3_.maleLook.split("|")[1]).split(",")[0];
               _skinsForBreed[_loc4_] = _loc3_.id;
               _skinsForBreed[SkinMapping.getSkinMappingById(int(_loc4_)).lowDefId] = _loc3_.id;
               _loc4_ = (_loc4_ = _loc3_.femaleLook.split("|")[1]).split(",")[0];
               _skinsForBreed[_loc4_] = _loc3_.id;
               _skinsForBreed[SkinMapping.getSkinMappingById(int(_loc4_)).lowDefId] = _loc3_.id;
            }
         }
         for(_loc2_ in _skinsForBreed)
         {
            if(_loc2_ == param1.toString())
            {
               _loc5_ = _skinsForBreed[_loc2_];
            }
         }
         return GameData.getObject(MODULE,_loc5_) as Breed;
      }
